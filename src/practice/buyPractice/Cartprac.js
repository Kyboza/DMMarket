...
const Cart = () => {
    ...
    const clearFormData = useStoreActions((actions) => actions.cart.clearFormData);
    ...
    const initializeCart = useStoreActions((actions) => actions.cart.initializeCart);
    
    const navigate = useNavigate();
    const totalCartAmount = cartItems ? cartItems.reduce((total, card) => total + card.amount, 0) : 0;
    ...
    const calculateTotal = () => {
        return cartItems.reduce((total, card) => total + card.price * card.amount, 0).toFixed(2);
    };
    ...
    const removeCopy = (index) => {
        ...
        if (specificCard) {
            ...
            if (cardAmount > 1) {
                ...
                console.log('Card amount is 1, prompting to remove');
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        // Call removeItem to remove the card from the cart
                        removeItem(index);
                    } else {
                        Swal.fire({
                            title: 'No Problem',
    ...
    return (
        <main className='Main__container'>
            ...
            <div className='Cart__buttons_container'>
                ...
                <button className='Cart__button' onClick={() => cartProceed(cartItems)}>Checkout</button>
